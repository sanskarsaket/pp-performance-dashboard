...
C.drillSpend = function (tree) {
  const top = tree.slice(0, 10);
  const platSet = [...new Set(top.flatMap(p => p.children.map(c => c.key)))];
  const colFor = p => (p === 'Google' ? COL.google : p === 'Meta' ? COL.meta : COL.other);
  return C.render('chDrillSpend', {
    type: 'bar',
    data: {
      labels: top.map(p => (p.key.length > 26 ? p.key.slice(0, 25) + '…' : p.key)),
      datasets: platSet.map(pl => ({
        label: pl,
        data: top.map(p => { const c = p.children.find(x => x.key === pl); return c ? c.spend : 0; }),
        backgroundColor: colFor(pl), borderRadius: 3, stack: 's'
      }))
    },
    options: {
      indexAxis: 'y',
      scales: { x: { ...gridY(v => A.moneyShort(v)), stacked: true }, y: { stacked: true, grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5 } } } },
      plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { label: c => `${c.dataset.label}: ${A.money(c.parsed.x)}` } } }
    }
  });
};

C.drillEfficiency = function (tree) {
  const top = tree.filter(p => p.leads >= 3).slice(0, 12);
  return C.render('chDrillEff', {
    type: 'bar',
    data: {
      labels: top.map(p => (p.key.length > 24 ? p.key.slice(0, 23) + '…' : p.key)),
      datasets: [
        { label: 'CPL', data: top.map(p => p.cpl), backgroundColor: COL.google, borderRadius: 3, yAxisID: 'y' },
        { label: 'CPQL', data: top.map(p => p.cpql), backgroundColor: COL.warn, borderRadius: 3, yAxisID: 'y' }
      ]
    },
    options: {
      scales: { x: { ...gridX, ticks: { font: { size: 10 }, maxRotation: 40, minRotation: 40 } }, y: gridY(v => A.moneyShort(v)) },
      plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { label: c => `${c.dataset.label}: ${A.money(c.parsed.y)}` } } }
    }
  });
};
...
C.asmRatios = function (rows) {
  const r = rows.slice(0, 12);
  return C.render('chAsmRatios', {
    type: 'bar',
    data: {
      labels: r.map(x => (x.key.length > 22 ? x.key.slice(0, 21) + '…' : x.key)),
      datasets: [
        { label: 'Lead to QL', data: r.map(x => x.leadToQl || 0), backgroundColor: COL.google, borderRadius: 3 },
        { label: 'QL to visit', data: r.map(x => x.qlToVisit || 0), backgroundColor: COL.ok, borderRadius: 3 },
        { label: 'Visit to EOI', data: r.map(x => x.visitToEoi || 0), backgroundColor: COL.brand, borderRadius: 3 }
      ]
    },
    options: {
      scales: { x: { ...gridX, ticks: { font: { size: 10 }, maxRotation: 40, minRotation: 40 } }, y: gridY(v => v + '%') },
      plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { label: c => `${c.dataset.label}: ${c.parsed.y.toFixed(1)}%` } } }
    }
  });
};

C.asmVolume = function (rows) {
  const r = rows.slice(0, 12);
  return C.render('chAsmVolume', {
    type: 'bar',
    data: {
      labels: r.map(x => (x.key.length > 22 ? x.key.slice(0, 21) + '…' : x.key)),
      datasets: [
        { label: 'Qualified', data: r.map(x => x.qualified), backgroundColor: COL.ok, stack: 'a', borderRadius: 3 },
        { label: 'TBD', data: r.map(x => x.tbd), backgroundColor: COL.google, stack: 'a', borderRadius: 3 },
        { label: 'Unqualified', data: r.map(x => x.unqualified), backgroundColor: COL.other, stack: 'a', borderRadius: 3 }
      ]
    },
    options: {
      indexAxis: 'y',
      scales: { x: { ...gridY(), stacked: true }, y: { stacked: true, grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5 } } } },
      plugins: { legend: { position: 'bottom' } }
    }
  });
};
...
C.auditRules = function (byRule) {
  const sevCol = { critical: COL.brand, major: COL.warn, minor: COL.other };
  return C.render('chAuditRules', {
    type: 'bar',
    data: {
      labels: byRule.map(r => r.label),
      datasets: [{ label: 'Leads flagged', data: byRule.map(r => r.count), backgroundColor: byRule.map(r => sevCol[r.sev]), borderRadius: 3 }]
    },
    options: {
      indexAxis: 'y',
      scales: { x: gridY(), y: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5 } } } },
...
C.auditOwners = function (owners) {
  const r = owners.slice(0, 14);
  return C.render('chAuditOwners', {
    type: 'bar',
    data: {
      labels: r.map(x => (x.key.length > 20 ? x.key.slice(0, 19) + '…' : x.key)),
      datasets: [{
        label: 'Audit score',
        data: r.map(x => x.score),
        backgroundColor: r.map(x => (x.score >= 80 ? COL.ok : x.score >= 60 ? COL.warn : COL.brand)),
        borderRadius: 3
      }]
    },
    options: {
      scales: { x: { ...gridX, ticks: { font: { size: 10 }, maxRotation: 40, minRotation: 40 } }, y: { ...gridY(), max: 100 } },
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: c => `Score ${c.parsed.y.toFixed(0)} of 100` } } }
    }
  });
};

C.contactMix = function (mix, labels) {
  const rows = mix.rows.filter(r => r.count > 0);
  const colMap = { engaged: COL.ok, visit: '#0891B2', callback: COL.google, noanswer: COL.warn, whatsapp: COL.meta, dead: COL.brand, other: COL.other };
  return C.render('chContactMix', {
    type: 'doughnut',
    data: {
      labels: rows.map(r => labels[r.tag] || r.tag),
      datasets: [{ data: rows.map(r => r.count), backgroundColor: rows.map(r => colMap[r.tag] || COL.other), borderWidth: 2, borderColor: '#fff' }]
    },
    options: {
      cutout: '60%',
      plugins: {
        ...
        tooltip: { callbacks: { label: c => {
          const tot = c.dataset.data.reduce((a, b) => a + b, 0);
          return `${c.label}: ${A.n(c.parsed)} entries (${tot ? ((c.parsed / tot) * 100).toFixed(1) : 0}%)`;
        } } }
      }
    }
  });
